...
{#if open}
	<div
		class="fixed inset-0 z-40 flex justify-end bg-slate-950/50"
		role="dialog"
		aria-modal="true"
	...
	>
		<aside
			class="flex h-full w-full max-w-[52rem] flex-col border-l shadow-2xl"
			style="border-color: var(--hub-border); background: color-mix(in srgb, var(--hub-surface) 97%, white);"
		>
			<header
			...
			>
				<div>
					...
					<p class="mt-2 max-w-2xl text-sm" style="color: var(--hub-muted);">
						Start OpenCode Web on the target machine first, make it reachable, then paste the URL
						into Add Server.
					</p>
				</div>
				<button
			...
			<div class="flex-1 overflow-y-auto px-5 py-5">
				<div class="grid gap-4 md:grid-cols-[1.2fr_0.8fr]">
					<section class="space-y-4">
						...
						>
							<h3 class="text-sm font-semibold" style="color: var(--hub-text);">
								What this page expects
							</h3>
							<p class="mt-2 text-sm" style="color: var(--hub-muted);">
								OpenCode Web must already be running on the target machine before you add it here.
								Start OpenCode Web, make it reachable, then paste its URL into Add Server.
							</p>
							<p class="mt-2 text-sm" style="color: var(--hub-muted);">
						...
						>
							...
							<div class="mt-3 space-y-3">
								...
								>
									...
									<p class="mt-1 text-sm" style="color: var(--hub-muted);">
										Use this when you have a reverse proxy or public TLS endpoint in front of
										OpenCode Web.
									</p>
								</div>
							</div>
					...
					<section class="space-y-4">
						<div
							...
							style="border-color: var(--hub-border); background: var(--hub-surface);"
						>
							<h3 class="text-sm font-semibold" style="color: var(--hub-text);">Security notes</h3>
							<ul class="mt-2 space-y-2 text-sm" style="color: var(--hub-muted);">
								...
								<li>
									Use trusted-IP restrictions where possible, and prefer HTTPS or a reverse proxy
									for wider exposure.
								</li>
							...
							<p class="mt-2 text-sm" style="color: var(--hub-muted);">
								Choose a strong password and avoid exposing a passwordless server to untrusted
								networks.
							</p>
						</div>
						...
						>
							...
							<p class="mt-2 text-sm" style="color: var(--hub-muted);">
								Save the LaunchAgent to <code
									>~/Library/LaunchAgents/com.user.opencode-web.plist</code
								>.
							</p>
							<CopyableCodeBlock label="LaunchAgent plist" language="xml" code={launchdExample} />
							<CopyableCodeBlock
								label="launchctl commands"
								language="bash"
								code={launchctlCommands}
							/>
						...
						>
							...
							<p class="mt-2 text-sm" style="color: var(--hub-muted);">
								`YOUR_CLIENT_IP` is the IP of the machine running this app. iptables rules may need
								to be saved separately to persist after reboot.
							</p>
							<p class="mt-2 text-sm" style="color: var(--hub-muted);">
						...
						<div
							...
							style="border-color: var(--hub-border); background: var(--hub-surface);"
						>
							<h3 class="text-sm font-semibold" style="color: var(--hub-text);">Final checklist</h3>
							<ul class="mt-2 space-y-2 text-sm" style="color: var(--hub-muted);">
